...
export async function POST(_request: Request, { params }: RouteParams) {
  ...
  try {
    const result = await reprocess.archive(session.user.id, id);
    if (!result) {
    ...
    console.error("[import/documents/archive]", error);
    return NextResponse.json({ error: "Falha ao arquivar documento" }, { status: 500 });
  }
}
