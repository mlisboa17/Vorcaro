...
import { investmentTxSchema } from "@/lib/patrimony/patrimony-schemas";
import {
...
export async function POST(request: Request) {
  ...
  const parsed = investmentTxSchema.safeParse(body);

  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }

  const { registerInvestment } = buildPatrimonyUseCases();

  try {
    const result = await registerInvestment.execute({
